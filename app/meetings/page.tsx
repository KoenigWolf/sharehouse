"use client";

import { useMemo } from "react";
import {
  NotebookPen,
  Sparkles,
  ListChecks,
  Clock3,
  Users,
  ArrowRight,
  CalendarClock,
} from "lucide-react";
import { PageContainer } from "@/src/shared/layouts";
import { MeetingNotesList, useMeetingNotes } from "@/src/features/meetings";
import { useLanguage } from "@/src/shared/lang/context";
import { Badge } from "@/components/ui/badge";
import { Card } from "@/components/ui/card";
import { cn } from "@/src/lib/utils";
import { format } from "date-fns";

export default function MeetingNotesPage() {
  const { lang } = useLanguage();
  const { notes, loading, error } = useMeetingNotes();

  const stats = useMemo(() => {
    const decisions = notes.reduce((sum, note) => sum + note.decisions.length, 0);
    const actionItems = notes.reduce((sum, note) => sum + note.actionItems.length, 0);
    const latestDate = notes[0]?.date ? format(new Date(notes[0].date), "yyyy/MM/dd") : "—";
    return { entries: notes.length, decisions, actionItems, latestDate };
  }, [notes]);

  return (
    <PageContainer>
      <div className="relative">
        <div
          className={cn(
            "absolute inset-0 -z-10",
            "bg-linear-to-b from-indigo-50/80 via-purple-50/50 to-transparent",
            "dark:from-indigo-950/40 dark:via-slate-950/70 dark:to-transparent"
          )}
          aria-hidden="true"
        />

        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-10 lg:py-14 space-y-8 sm:space-y-10">
          <HeroSection lang={lang} stats={stats} />

          {loading && (
            <div className="text-sm text-slate-500 dark:text-slate-400">
              {lang.pages.meetings.loading}
            </div>
          )}

          {error && (
            <div
              className="text-sm rounded-xl border border-red-200 dark:border-red-800/50 bg-red-50 dark:bg-red-950/30 px-4 py-3 text-red-600 dark:text-red-400"
            >
              {lang.common.errorPrefix} meeting notes: {error.message}
            </div>
          )}

          {!loading && !error && (
            <div className="grid gap-6 lg:gap-10 lg:grid-cols-[1.7fr,1fr]">
              <section className="space-y-4">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <p className="text-xs font-semibold tracking-[0.14em] uppercase text-indigo-600 dark:text-indigo-300">
                      {lang.pages.meetings.eyebrow}
                    </p>
                    <h2 className="mt-1 text-lg sm:text-xl font-semibold text-slate-900 dark:text-white">
                      {lang.pages.meetings.title}
                    </h2>
                  </div>
                  <Badge className="bg-slate-900 text-white dark:bg-white dark:text-slate-900">
                    {lang.pages.meetings.notes}
                  </Badge>
                </div>

                <MeetingNotesList notes={notes} />
              </section>

              <AsidePanel lang={lang} />
            </div>
          )}
        </div>
      </div>
    </PageContainer>
  );
}

interface HeroSectionProps {
  lang: ReturnType<typeof useLanguage>["lang"];
  stats: {
    entries: number;
    decisions: number;
    actionItems: number;
    latestDate: string;
  };
}

function HeroSection({ lang, stats }: HeroSectionProps) {
  return (
    <section
      className={cn(
        "relative overflow-hidden rounded-3xl border border-slate-200/70 dark:border-slate-800/70",
        "bg-white/80 dark:bg-slate-900/70 backdrop-blur-xl",
        "shadow-[0_25px_80px_-40px] shadow-indigo-500/30"
      )}
    >
      <div className="absolute inset-0 opacity-70" aria-hidden="true">
        <div className="absolute -left-10 sm:-left-16 top-0 w-40 sm:w-56 h-40 sm:h-56 bg-linear-to-br from-indigo-500/20 via-sky-400/20 to-teal-400/10 blur-3xl" />
        <div className="absolute right-0 -bottom-16 w-48 sm:w-64 h-48 sm:h-64 bg-linear-to-tr from-amber-400/20 via-rose-400/15 to-indigo-500/10 blur-3xl" />
      </div>

      <div className="relative grid gap-8 lg:gap-10 lg:grid-cols-[1.35fr,1fr] p-6 sm:p-8 lg:p-10">
        <div className="space-y-5">
          <div className="flex items-center gap-3">
            <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-linear-to-br from-indigo-600 via-sky-500 to-emerald-500 text-white shadow-lg shadow-indigo-500/30">
              <NotebookPen className="h-6 w-6" strokeWidth={2.25} />
            </div>
            <Badge className="bg-slate-900 text-white dark:bg-white dark:text-slate-900 shadow-lg shadow-indigo-500/20">
              {lang.pages.meetings.eyebrow}
            </Badge>
          </div>

          <div className="space-y-3">
            <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold leading-tight text-slate-900 dark:text-white">
              {lang.pages.meetings.title}
            </h1>
            <p className="text-base sm:text-lg text-slate-600 dark:text-slate-400 leading-relaxed max-w-2xl">
              {lang.pages.meetings.description}
            </p>
            <p className="text-base sm:text-lg text-slate-700 dark:text-slate-200 leading-relaxed max-w-2xl">
              {lang.pages.meetings.heroSub}
            </p>
          </div>

          <div className="flex flex-wrap gap-2 sm:gap-3">
            {lang.pages.meetings.highlightItems.map((item) => (
              <Badge
                key={item}
                variant="outline"
                className="border-slate-200/80 bg-white/70 text-slate-700 dark:border-slate-700 dark:bg-slate-800/80 dark:text-slate-100"
              >
                <Sparkles className="h-3.5 w-3.5 text-indigo-500" />
                {item}
              </Badge>
            ))}
          </div>
        </div>

        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3 sm:gap-4">
            <StatCard
              icon={<ListChecks className="h-4 w-4" />}
              label={lang.pages.meetings.stats.entries}
              value={stats.entries}
              accent="from-indigo-500 to-purple-500"
            />
            <StatCard
              icon={<Users className="h-4 w-4" />}
              label={lang.pages.meetings.stats.decisions}
              value={stats.decisions}
              accent="from-emerald-500 to-teal-500"
            />
            <StatCard
              icon={<Sparkles className="h-4 w-4" />}
              label={lang.pages.meetings.stats.actionItems}
              value={stats.actionItems}
              accent="from-amber-500 to-orange-500"
              compact
            />
            <StatCard
              icon={<CalendarClock className="h-4 w-4" />}
              label={lang.pages.meetings.stats.nextMeeting}
              value={stats.latestDate}
              accent="from-slate-900 to-slate-700"
              compact
            />
          </div>

          <Card className="border-0 bg-linear-to-r from-slate-900 via-slate-800 to-indigo-900 text-white shadow-xl shadow-indigo-500/30">
            <div className="flex items-start gap-3 p-4 sm:p-5">
              <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-white/10">
                <ArrowRight className="h-4 w-4" />
              </div>
              <div>
                <p className="text-sm font-semibold tracking-wide uppercase text-white/70">
                  {lang.pages.meetings.highlightTitle}
                </p>
                <p className="mt-1 text-base sm:text-lg font-medium leading-relaxed">
                  {lang.pages.meetings.heroNote}
                </p>
              </div>
            </div>
          </Card>
        </div>
      </div>
    </section>
  );
}

interface StatCardProps {
  icon: React.ReactNode;
  label: string;
  value: number | string;
  accent: string;
  compact?: boolean;
}

function StatCard({ icon, label, value, accent, compact }: StatCardProps) {
  return (
    <div
      className={cn(
        "rounded-2xl border border-slate-200/80 dark:border-slate-800/70",
        "bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm",
        "shadow-md shadow-indigo-500/10",
        "p-3 sm:p-4",
        compact ? "col-span-2 sm:col-span-1" : ""
      )}
    >
      <div className="flex items-center gap-3">
        <div className={cn("h-9 w-9 rounded-xl text-white flex items-center justify-center bg-linear-to-br", accent)}>
          {icon}
        </div>
        <div className="min-w-0">
          <p className="text-xs uppercase tracking-wide text-slate-500 dark:text-slate-400 font-semibold">
            {label}
          </p>
          <p className="text-lg sm:text-xl font-semibold text-slate-900 dark:text-white leading-tight break-words">
            {value}
          </p>
        </div>
      </div>
    </div>
  );
}

interface AsidePanelProps {
  lang: ReturnType<typeof useLanguage>["lang"];
}

function AsidePanel({ lang }: AsidePanelProps) {
  return (
    <aside className="space-y-4">
      <Card className="overflow-hidden border-slate-200/70 dark:border-slate-800/70 bg-white/80 dark:bg-slate-900/70">
        <div className="p-5 sm:p-6">
          <div className="flex items-center gap-3">
            <div className="h-10 w-10 rounded-xl bg-linear-to-br from-indigo-500 to-sky-500 text-white flex items-center justify-center shadow-lg shadow-indigo-500/20">
              <Clock3 className="h-5 w-5" />
            </div>
            <div>
              <p className="text-xs uppercase tracking-[0.2em] text-indigo-600 dark:text-indigo-300 font-semibold">
                {lang.pages.meetings.sidebarTitle}
              </p>
              <p className="text-sm text-slate-500 dark:text-slate-400">
                {lang.pages.meetings.sidebarDescription}
              </p>
            </div>
          </div>

          <div className="mt-4 space-y-3">
            {lang.pages.meetings.sidebarChecks.map((check) => (
              <div
                key={check}
                className="flex items-start gap-3 rounded-xl border border-slate-200/80 dark:border-slate-800/70 bg-slate-50/70 dark:bg-slate-800/60 px-3.5 py-3"
              >
                <span className="mt-1 h-2 w-2 rounded-full bg-emerald-500 shadow-[0_0_0_4px] shadow-emerald-500/20" />
                <p className="text-sm text-slate-700 dark:text-slate-200 leading-relaxed">{check}</p>
              </div>
            ))}
          </div>

          <div className="mt-5 rounded-2xl bg-linear-to-r from-indigo-500 to-slate-900 text-white px-4 py-3 shadow-md shadow-indigo-500/20">
            <p className="text-sm font-semibold">{lang.pages.meetings.sidebarCTA}</p>
            <p className="text-xs text-white/80 mt-1">{lang.pages.meetings.sidebarCTANote}</p>
          </div>
        </div>
      </Card>
    </aside>
  );
}
