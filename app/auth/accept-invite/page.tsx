...
/**
 * Accept Invite Page
 * Allows invited users to set up their account
 */
...
import { Home, Loader2, CheckCircle, UserPlus } from "lucide-react";
...
export default function AcceptInvitePage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : lang.pages.login.errorMessage
      );
    } finally {
      setIsLoading(false);
    }
        ...
        <div className="bg-white dark:bg-slate-800/80 rounded-3xl shadow-2xl border border-slate-100 dark:border-slate-700/50 p-8 backdrop-blur-sm">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-muted dark:text-subtle mt-2">
              {viewState === "success"
                ? lang.pages.acceptInvite.success
                : lang.pages.acceptInvite.subtitle}
            </p>
          </div>
            ...
            <div className="space-y-6">
              <div className="p-6 rounded-xl bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 text-center">
                ...
                <p className="text-sm text-emerald-700 dark:text-emerald-300">
                  {lang.pages.acceptInvite.successDescription}
                </p>
              </div>
              <Link href="/" className="block">
                <Button className="w-full h-12">
                  {lang.pages.acceptInvite.goToHome}
                </Button>
              </Link>
            </div>
          )}
          ...
          {viewState === "form" && (
            <form onSubmit={handleSubmit} className="space-y-6" suppressHydrationWarning>
              <Input
                id="nickname"
                type="text"
                label={lang.pages.acceptInvite.nicknameLabel}
                value={nickname}
                onChange={(e) => setNickname(e.target.value)}
                required
                autoComplete="nickname"
                placeholder={lang.pages.acceptInvite.nicknamePlaceholder}
              />

              <Input
                id="password"
                type="password"
                label={lang.pages.acceptInvite.passwordLabel}
                value={password}
                onChange={(e) => setPassword(e.target.value)}
                required
                autoComplete="new-password"
                placeholder={lang.pages.acceptInvite.passwordPlaceholder}
              />

              <Input
                id="confirmPassword"
                type="password"
                label={lang.pages.acceptInvite.confirmPasswordLabel}
                value={confirmPassword}
                onChange={(e) => setConfirmPassword(e.target.value)}
                required
                autoComplete="new-password"
                placeholder={lang.pages.acceptInvite.confirmPasswordPlaceholder}
              />
              ...
              <Button type="submit" className="w-full h-12" disabled={isLoading}>
                {isLoading ? (
                  <>
                    ...
                    {lang.pages.acceptInvite.creating}
                  </>
                ) : (
                  <>
                    <UserPlus className="w-4 h-4 mr-2" />
                    {lang.pages.acceptInvite.createAccount}
                  </>
                )}
              </Button>
            </form>
